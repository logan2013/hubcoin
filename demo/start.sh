...
cd $GOPATH/src/github.com/dragosroua/hubcoin/demo
...
# grab the chain ids
CHAIN_ID1=$(cat ./data/chain1/hubcoin/genesis.json | jq .[1])
CHAIN_ID1=$(removeQuotes $CHAIN_ID1)
CHAIN_ID2=$(cat ./data/chain2/hubcoin/genesis.json | jq .[1])
CHAIN_ID2=$(removeQuotes $CHAIN_ID2)
echo "CHAIN_ID1: $CHAIN_ID1"
echo "CHAIN_ID2: $CHAIN_ID2"

# make reusable chain flags
CHAIN_FLAGS1="--chain_id $CHAIN_ID1 --from ./data/chain1/hubcoin/key.json"
CHAIN_FLAGS2="--chain_id $CHAIN_ID2 --from ./data/chain2/hubcoin/key.json --node tcp://localhost:36657"
...
hubcoin start --dir ./data/chain1/hubcoin &> chain1_hubcoin.log &
...
hubcoin start --address tcp://localhost:36658 --dir ./data/chain2/hubcoin &> chain2_hubcoin.log &
...
hubcoin tx ibc --amount 10mycoin $CHAIN_FLAGS2 register --chain_id $CHAIN_ID1 --genesis ./data/chain1/tendermint/genesis.json
...
hubcoin tx ibc --amount 10mycoin $CHAIN_FLAGS1 packet create --from $CHAIN_ID1 --to $CHAIN_ID2 --type coin --payload $PAYLOAD --sequence 1

echo ""
echo "... querying for packet data"
echo ""
# query for the packet data and proof
QUERY_RESULT=$(hubcoin query ibc,egress,$CHAIN_ID1,$CHAIN_ID2,1)
...
HEADER_AND_COMMIT=$(hubcoin block $HEIGHT) 
...
hubcoin tx ibc --amount 10mycoin $CHAIN_FLAGS2 update --header 0x$HEADER --commit 0x$COMMIT

echo ""
echo "... posting packet from chain1 on chain2"
echo ""
# post the packet from chain1 to chain2
hubcoin tx ibc --amount 10mycoin $CHAIN_FLAGS2 packet post --from $CHAIN_ID1 --height $HEIGHT --packet 0x$PACKET --proof 0x$PROOF

echo ""
echo "... checking if the packet is present on chain2"
echo ""
# query for the packet on chain2
hubcoin query --node tcp://localhost:36657 ibc,ingress,test_chain_2,test_chain_1,1
